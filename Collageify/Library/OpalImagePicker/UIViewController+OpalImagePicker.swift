...
public extension UIViewController {
    ...
    func presentOpalImagePickerController(_ imagePicker: OpalImagePickerController, animated: Bool, select: @escaping (([PHAsset], [[String: Any]]) -> Void), cancel: @escaping (() -> Void), completion: (() -> Void)? = nil) {
    ...
    ///   - cancel: notifies when Image Picker has been cancelled by user
    ///   - completion: notifies when the Image Picker finished presenting
    func presentOpalImagePickerController(_ imagePicker: OpalImagePickerController, animated: Bool, maximumSelectionsAllowed: Int = 10, numberOfExternalItems: Int, externalItemsTitle: String, externalURLForIndex: @escaping (Int) -> URL?, selectImages: (([UIImage]) -> Void)? = nil, selectAssets: (([PHAsset], [[String: Any]]) -> Void)? = nil, selectExternalURLs: (([URL]) -> Void)? = nil, cancel: @escaping () -> Void, completion: (() -> Void)? = nil) {
...
class OpalImagePickerManager: NSObject {
    var selectAssets: (([PHAsset], [[String: Any]]) -> Void)?
    var cancel: (() -> Void)?
...
extension OpalImagePickerManager: OpalImagePickerControllerDelegate {
    func imagePicker(_ picker: OpalImagePickerController, didFinishPickingAssets assets: [PHAsset], allImages: [[String: Any]]) {
        selectAssets?(assets, allImages)
    }
