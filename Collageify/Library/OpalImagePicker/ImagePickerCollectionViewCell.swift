...
class BottomImageCollectionViewCell: UICollectionViewCell {
    
    static let reuseId = String(describing: BottomImageCollectionViewCell.self)
    ...
    override init(frame: CGRect) {
        ...
        let constraintsToFillPlaceHolder = contentView.constraintsToFill(otherView: placeHolderImageView)
        NSLayoutConstraint.activate(constraintsToFill + constraintsToFillPlaceHolder)
        

        deleteButton.setImage(UIImage(named: "img_delete"), for: .normal)
...
//    }
    ...
    private func addOverlay(_ animated: Bool) {
        ...
        overlayImageView.image = selectionImage ?? UIImage(named: "checkmark", in: Bundle.podBundle(forClass: type(of: self).self), compatibleWith: nil)?.withRenderingMode(.alwaysOriginal)
        overlayImageView.tintColor = selectionImageTintColor
        overlayImageView.alpha = 0
        contentView.addSubview(overlayImageView)
        self.overlayImageView = overlayImageView
        
        overlayImageView.topAnchor.constraint(equalTo: contentView.topAnchor, constant: 5).isActive = true
        overlayImageView.trailingAnchor.constraint(equalTo: contentView.trailingAnchor, constant: -5).isActive = true
        overlayImageView.heightAnchor.constraint(equalToConstant: 22).isActive = true
        overlayImageView.widthAnchor.constraint(equalToConstant: 22).isActive = true
