...
open class CropperViewController: UIViewController, Rotatable, StateRestorable, Flipable {
    ...
    open override func viewWillTransition(to size: CGSize, with coordinator: UIViewControllerTransitionCoordinator) {
        if !view.sizee.isEqual(to: size, accuracy: 0.0001) {
            needReload = true
        }
...
// MARK: - Private Methods
    ...
    open func resetToDefaultLayout() {
        ...
        toolbar.sizee = CGSize(width: view.width, height: view.safeAreaInsets.bottom + barHeight)
        bottomView.sizee = CGSize(width: view.width, height: toolbar.height + angleRuler.height + margin)
